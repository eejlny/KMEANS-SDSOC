...
void clustering(uint n, uint k, centre_type *initial_centres, data_type_short *data_points, kdTree_type *tree_memory, uint root, data_type_short *output_array, centre_type *centre_output)
{
    ...
    coord_type_short value_tmp[k][D];
        ...
        for (uint i=0; i<k; i++) {
            ...
            for (uint d=0; d<D; d++) {
                if (tmp_count_short != 0) {
                			...
                			value_tmp[i][d] = (coord_type_short)(centres[i].wgtCent.value[d]/((coord_type)tmp_count_short));
                			if (centres[i].position_short.value[d] != value_tmp[i][d])
                			{
                				centres[i].position_short.value[d]=value_tmp[i][d];
                				exit_loop = false;
                			}
                		//	printf("Centers are %d\n",(int)value_tmp[i][d]);
                	#else
                			centres[i].position_short.value[d]= (coord_type_short)(centres[i].wgtCent.value[d]/((coord_type)tmp_count_short));
                		//	printf("Centers are %d\n",(int)centres[i].position_short.value[d]);
                	#endif
                }
...
/*
        printf("Intermediate centres \n");
            for (uint i=0; i<k; i++) {
                printf("%d: ",i);
                for (uint d=0; d<D-1; d++) {
                    printf("%d ",centres[i].position_short.value[d]);
                }
                printf("%d\n",centres[i].position_short.value[D-1]);
            }
            printf("Exit loop is %d\n",exit_loop);
*/
